        ...
        this._update = function (refactor) {
            ...
            var distinctCats = [],
                ...
                getOrderedCategories = function (self, axPos, oppPos) {
                    ...
                    definitions = self._orderRules.concat({
                        ordering: sortBy,
                        desc: desc
                    });
    ...
    dimple.chart = function (svg, data) {
        ...
        this._getData = function (data, cats, agg, order, stacked, x, y, z, p, c) {
            ...
            if (x && x._hasCategories() && x._hasMeasure()) {
                ...
                xSortArray = dimple._getOrderedList(sortedData, xCat, x._orderRules.concat([{
                    ordering: x.measure,
                    desc: true
                }]));
            }
            if (y && y._hasCategories() && y._hasMeasure()) {
                ...
                ySortArray = dimple._getOrderedList(sortedData, yCat, y._orderRules.concat([{
                    ordering: y.measure,
                    desc: true
                }]));
            }
            if (p && p._hasCategories() && p._hasMeasure()) {
                ...
                pSortArray = dimple._getOrderedList(sortedData, pCat, p._orderRules.concat([{
                    ordering: p.measure,
                    desc: true
                }]));
            }
            ...
            for (key in totals.x) {
                if (totals.x.hasOwnProperty(key)) {
                    grandTotals.x += totals.x[key];
                }
            }
            for (key in totals.y) {
                if (totals.y.hasOwnProperty(key)) {
                    grandTotals.y += totals.y[key];
                }
            }
            for (key in totals.p) {
                if (totals.p.hasOwnProperty(key)) {
                    grandTotals.p += totals.p[key];
                }
            }
            for (key in totals.z) {
                if (totals.z.hasOwnProperty(key)) {
        ...
        this.draw = function (duration, noDataChange) {
            ...
            this.axes.forEach(function (axis) {
                ...
                if (axis._hasMeasure()) {
                    ...
                    this.series.forEach(function (series) {
                        // if this axis is linked
                        if (series._deepMatch(axis) && series[axis.position].timeField !== null && series[axis.position].timeField !== undefined && linkedDimensions.indexOf(series[axis.position].timeField) === -1) {
                    ...
                    this.series.forEach(function (series) {
                        // if this axis is linked
                        if (series._deepMatch(axis) && series[axis.position].categoryFields[0] !== null && series[axis.position].categoryFields[0] !== undefined && linkedDimensions.indexOf(series[axis.position].categoryFields[0]) === -1) {
                    ...
                    if (!chart.noFormats) {
                        this.attr("opacity", function (d) {
                            ...
                            .style("fill", function (d) {
                                return (graded ? "url(#fill-area-gradient-" + d.keyString + ")" : d.color.fill);
                            })
                            .style("stroke", function (d) {
                                return (graded ? "url(#stroke-area-gradient-" + d.keyString + ")" : d.color.stroke);
                            })
                ...
                .call(function () {
                    ...
                    if (!chart.noFormats) {
                        this.attr("opacity", function (d) {
                            ...
                            .style("fill", "none")
                            .style("stroke", function (d) {
                                return (graded ? "url(#fill-line-gradient-" + d.keyString + ")" : d.color.stroke);
                            })
                            .style("stroke-width", series.lineWeight);
                    }
                })
    ...
    dimple._getSeriesOrder = function (data, series) {
        ...
        if (cats !== null && cats !== undefined && cats.length > 0) {
            if (series.c !== null && series.c !== undefined && series.c._hasMeasure()) {
                rules.push({
                    ordering: series.c.measure,
                    desc: true
                });
            }
            if (series.x._hasMeasure()) {
                rules.push({
                    ordering: series.x.measure,
                    desc: true
                });
            }
            if (series.y._hasMeasure()) {
                rules.push({
                    ordering: series.y.measure,
                    desc: true
                });
            }
